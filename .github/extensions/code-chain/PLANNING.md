...
- **File ownership is exclusive.** Every chunk owns a disjoint set of files — no two chunks write the same file, especially within a wave (a shared file means a guaranteed merge conflict). The plan engineers this by structure: per-entity modules, one router/module file per chunk, and an auto-include/registration seam so chunks never co-edit a shared app/models/router-registry file. Unavoidable shared scaffold is created once in an early width-1 wave that later waves only import, never edit.
