...
- **Cut waves along domain seams; never fracture a cohesive unit to manufacture parallelism.** Parallelism is *harvested* from the dependency graph, not *forced* by splitting one resource's operations across files just to make them disjoint. If two operations belong to the same resource and would naturally live in one file (e.g. create/list and an update/patch of the same entity), keep them together and build them in one chunk on a sequential wave — do **not** peel one operation into its own file solely so it can run in a parallel wave. The build strategy must leave no trace in the product: a reader should not be able to reverse-engineer the wave plan from how the files were divided, and the result must read as if written by one author. When cohesion and parallelism conflict, cohesion wins.
