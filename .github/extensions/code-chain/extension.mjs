...
Run the task sub-agents directly on your own worktree:
  ...
  task({ name: "build-chunk", agent_type: "general-purpose", model: "gpt-5.3-codex", mode: "sync",
         ...
         prompt: "FIRST read the coding baseline at ${CODING_DOC} (and ./CODING.md at the repo root if it exists) and the project's ./CONSTITUTION.md at the repo root if present, and conform to both. Implement ONLY this chunk: <chunk>. Touch only its OWNED files plus their tests. INSTRUMENT AS YOU BUILD — LOGGING IS A REQUIRED DELIVERABLE OF THIS CHUNK, not optional polish and not a follow-up: assume NO human will ever step through this code with a debugger, so the diagnostic logging that a human would otherwise rely on MUST already be present in the code you ship. Whenever you implement non-trivial control flow (error and early-return paths, branches, multi-step operations, or any call that crosses a service/process/network boundary) add structured logging in the SAME change so any failure is explainable from logs ALONE — without reading the code, eyeballing raw output, or correlating two processes by timestamp. Every non-success returned to a caller gets a server-side log line naming the machine reason; a cross-service call logs the callee's reason + correlation/request id, not just its status. For the CONCRETE logging MECHANISM — which logger/library to import, how to obtain it, level meanings, field names, and redaction rules — consult the LOCAL PROJECT's own ./CONSTITUTION.md (and ./CODING.md) at the repo root; that is the source of truth for HOW to log here, NOT code-chain's own docs. Use exactly the mechanism it specifies; be generous at its verbose/debug level; if the project's constitution states no logging convention, say so explicitly in your chunk notes rather than inventing a divergent one. Do NOT instrument trivial pure functions. Commit per logical step with conventional-commit messages. Then run this chunk's acceptance check and report pass/fail. Earlier chunks are already built and committed — do NOT rebuild them. Final chunk plan for context: <final plan>" })
...
worktree branched off YOUR branch:
  ...
  create_session({ project_id: "<THIS project's id>", base_branch: "<your current branch>",
     ...
     kickoff: { mode: "autopilot", model: "gpt-5.3-codex",
       prompt: "code-chain worker PARENT_RUN=<coord-run-id> — build EXACTLY ONE chunk and nothing else: <chunk>. Read the coding baseline at ${CODING_DOC} (and ./CODING.md if present) and ./CONSTITUTION.md if present; conform to both. Touch ONLY this chunk's OWNED files plus their tests — NEVER a file owned by another chunk. INSTRUMENT AS YOU BUILD — LOGGING IS A REQUIRED DELIVERABLE of this chunk, not optional: assume NO human will step through this code with a debugger, so the diagnostic logging must already be present. Add structured logging in the SAME change for non-trivial control flow (error/early-return paths, branches, multi-step ops, cross-service/process boundaries) so any failure is explainable from logs ALONE — every non-success returned to a caller gets a server-side log line naming the machine reason, and a cross-service call logs the callee's reason + correlation id, not just status. For the concrete logging MECHANISM (which logger to import, level meanings, field names, redaction rules) consult the LOCAL PROJECT's ./CONSTITUTION.md and ./CODING.md at the repo root — that is the source of truth for HOW to log here, NOT code-chain's docs; use exactly that mechanism, or note its absence in your chunk notes rather than inventing one. Commit per logical step (conventional commits). Run the chunk's acceptance check. THEN code-review your OWN diff \`git diff <wave-base>..HEAD\` with a SPEC-CONFORMANCE review: dispatch a code-review task with name \"review-chunk\" (the host REQUIRES the \`name\` field — a call without it is rejected with \`\\\"name\\\": Required\`), model \"claude-sonnet-5\" and reasoning_effort \"high\" — a DIFFERENT model family from you (the builder), so it re-derives from spec instead of sharing your blind spots. Hand the reviewer THIS chunk's spec rule(s) and instruct it to RE-DERIVE expected values from the spec, probe BOUNDARY inputs (half-cents, thresholds, off-by-one, tie-breaks), DISTRUST your own tests, treat any IMPLEMENTED-BUT-DIVERGENT rule as BLOCKING even when tests pass, AND apply the OBSERVABILITY GATE (logging is a required deliverable — request changes if a failure/error-to-caller path returns non-success with no structured log naming the reason, a multi-step/cross-service path lacks trace coverage at the repo's verbose level, a cross-service call logs only a bare status without callee reason + correlation id, or logging violates the repo's level/redaction rules OR uses an ad-hoc print / a logger other than the one the LOCAL project's ./CONSTITUTION.md mandates — but not for trivial pure functions). FIX any blocking issue, re-reviewing until green; if this chunk is HIGH-RISK, review per task. Do NOT merge and do NOT touch other chunks' files. When green + committed, do BOTH of these as your FINAL steps so your coordinator can detect completion even if a message is missed: (1) write a sentinel at your worktree root — \`printf 'CHUNK <id> tests=<pass|fail> branch=%s\\n' \"\$(git rev-parse --abbrev-ref HEAD)\" > .cc-done\` (do NOT commit it); (2) send your coordinator EXACTLY this message: 'CHUNK <id> DONE branch=<your branch> tests=<pass|fail>'." } })
