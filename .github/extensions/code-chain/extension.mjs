...
task({ name: "plan-critique", agent_type: "general-purpose", model: "claude-sonnet-4.6", reasoning_effort: "high", mode: "sync",
...
Run the task sub-agents directly on your own worktree:
  ...
  task({ name: "review-chunk", agent_type: "code-review", model: "claude-sonnet-4.6", reasoning_effort: "high", mode: "sync",
...
worktree branched off YOUR branch:
  ...
  task({ name: "integration-review", agent_type: "code-review", model: "claude-sonnet-4.6", reasoning_effort: "high", mode: "sync",
...
- Specify name + model + mode "sync" on every task call (the host REQUIRES \`name\` — a kebab label like plan-chunks/build-chunk/review-chunk — and rejects calls without it). Models are FIXED by ROLE: PLANNER="claude-opus-4.8" (medium), BUILDER="gpt-5.3-codex", REVIEWER="claude-sonnet-4.6" (high).
